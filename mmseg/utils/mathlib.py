...
def cos_similarity(x, y, dim, scale: torch.Tensor = None):
    x_norm = x / (x.norm(p=2, dim=dim, keepdim=True) + 1e-8)
    y_norm = y / (y.norm(p=2, dim=dim, keepdim=True) + 1e-8)
    cos = torch.sum(x_norm * y_norm, dim=dim)
