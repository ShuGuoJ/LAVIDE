...
def train_hybrid_segmentor(model,
                    dataset,
                    cfg,
                    distributed=False,
                    validate=False,
    ...
    num_of_data = len(dataset[0])
    if num_of_data % 2 == 0:
        dataset[0] = Subset(dataset[0], list(range(num_of_data // 2)))
        dataset[1] = Subset(dataset[1], list(range(num_of_data // 2, num_of_data)))
    else:
        dataset[0] = Subset(dataset[0], list(range(num_of_data // 2)))
        dataset[1] = Subset(dataset[1], list(range(num_of_data // 2, num_of_data - 1)))
    # dataset = [dataset[1]]
    data_loaders = [
        build_dataloader(
            ds,
