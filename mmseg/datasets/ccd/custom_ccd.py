...
class CustomDatasetCCD(CustomDatasetCD):
    ...
    def evaluate(self,
        ...
        SCD_metrics = ['BC', 'BC_precision', 'BC_recall', 'F1', 'NO_BC', 'NO_BC_precision', 'NO_BC_recall', 'NO_F1',
                       'mBC', 'mF1', 'SC', 'SCS', 'mIoU', 'SeK', 'Boundary', 'Inst_NO', 'Inst_Partial', 'Inst_Full', 'Inst_OA']
        ...
        # convert an array to individual scalars
        iou_per_class = ret_metrics.pop('IoU_per_class')
        sc_per_class = ret_metrics.pop('SC_per_class')
        for name, iou_cls, sc_cls in zip(list(class_names), list(iou_per_class), list(sc_per_class)):
            iou_name = 'IoU_' + name
            ret_metrics[iou_name] = iou_cls

            sc_name = 'SC_' + name
            ret_metrics[sc_name] = sc_cls
