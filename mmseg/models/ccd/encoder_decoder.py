...
class EncoderDecoderCCD(EncoderDecoder):
    ...
    def simple_test(self, img, img_metas, gt_semantic_seg_pre=None, rescale=True):
        ...
        # sem[bc==0] = gt_semantic_seg_pre.long()[bc==0]
        ...
        # (1) return feat for visualization
        # return {'bc': bc, 'sem': sem, 'feat': output['feat']}
    ...
    def encode_decode(self, img, img_metas, gt_semantic_seg_pre=None):
        ...
        # 尝试交换tile_inference和resize的顺序看看对变化检测的性能是否有影响
        # output['bc'] = resize(
        #     input=output['bc'],
        #     size=img.shape[2:],
        #     mode='bilinear',
...
@SEGMENTORS.register_module()
class DummyEncoderDecoderCCDWrapper(EncoderDecoderCCD):
    """Encoder Decoder wrapper for FLOPs calculation.
    
    This wrapper is designed for models that DO NOT require gt_semantic_seg_pre input.
    ...
    For models requiring gt_semantic_seg_pre, use DummyEncoderDecoderCMCDWrapper instead.
    ...
    def forward(self, img):
        ...
        are properly tracked by thop. Avoids the forward_test path which
        includes cpu().numpy() conversions that break thop's tracking.
        ...
        # Step 1: Extract features (backbone + neck)
        x = self.extract_feat(img_concat)
        
        # Step 2: Decode head forward
        # Handle different decode_head types:
        # - SemSegHeadCCD has sem_head.forward()
        # - SegformerHead has forward()
        # - Other heads may have different structures
        if hasattr(self.decode_head, 'sem_head'):
            # SemSegHeadCCD: use internal sem_head
            out = self.decode_head.sem_head.forward(x)
        elif hasattr(self.decode_head, 'forward'):
            # Standard head with forward method
            try:
                out = self.decode_head.forward(x)
            except NotImplementedError:
                # If forward is not implemented, try other methods
                if hasattr(self.decode_head, 'sem_head'):
                    out = self.decode_head.sem_head.forward(x)
                else:
                    raise
            ...
            raise NotImplementedError(
                f"decode_head type {type(self.decode_head).__name__} "
                "does not have a suitable forward method for FLOPs calculation"
            )
        
        return out

@SEGMENTORS.register_module()
class EncoderDecoderCCDSem(EncoderDecoderCCD):
    '''
    Overall model class for Conditional CD.
    '''
    def forward_train(self, img, img_metas, gt_semantic_seg, gt_semantic_seg_pre=None, gt_semantic_seg_post=None):
    ...
    def extract_feat(self, img, gt_semantic_seg_pre=None):
