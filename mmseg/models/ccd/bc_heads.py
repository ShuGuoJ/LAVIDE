...
class BaseHeadBC(BaseDecodeHead):
    ...
    def forward_test(
        ...
        # import os
        # root = '/remote-home/jiangshuguo/project/mapformer-master/hidden_feat'
        # data_name = 'dynamic_earth_net' # modification
        # model = 'ours' # modification
        # save_root = os.path.join(root, data_name, model)
        # os.makedirs(save_root, exist_ok=True)
        # f1_name, f2_name = img_metas[0]['filename1'], img_metas[0]['filename2']
        # f1_name, f2_name = os.path.splitext(f1_name)[0], os.path.splitext(f2_name)[0]
        # f1_name, f2_name = f1_name.split('/'), f2_name.split('/')
        # save_name = f'{f1_name[-2]}_{f1_name[-1]}_vs_{f2_name[-2]}_{f2_name[-1]}.pkl'
        # save_path = os.path.join(save_root, save_name)
        #
        # out, feat = self.forward(inputs=inputs, gt_semantic_seg_pre=gt_semantic_seg_pre)
        # feat = feat.cpu()
...
class SimpleHeadBC(BaseDecodeHead):
...
@HEADS.register_module()
class SimpleHeadBCD(BaseDecodeHead):
    def __init__(self, interpolate_mode='bilinear', **kwargs):
        super().__init__(input_transform='multiple_select', **kwargs)
    ...
    def forward_train(
        ...
        # assert len(inputs)==3, f'Expect 3 inputs, got {len(inputs)}'
        bc_logit, bc_feat = inputs[0], inputs[1]
        losses = self.losses(seg_logit=bc_logit, seg_label=gt_semantic_seg)
        return losses
    ...
    def forward_test(
        ...
        # assert len(inputs) == 3, f'Expect 3 inputs, got {len(inputs)}'
        bc_logit, bc_feat = inputs[0], inputs[1]
        return bc_logit, bc_feat


@HEADS.register_module()
class SimpleHeadBCUS(SimpleHeadBC):
    def forward_train(
            self,
            inputs,
            img_metas,
        ...
        # assert len(inputs)==3, f'Expect 3 inputs, got {len(inputs)}'
        if len(inputs) == 3:
            bc_logit, _, seg_logits_post = inputs
        elif len(inputs) == 2:
            bc_logit, seg_logits_post = inputs

        seg_label_post = seg_logits_post.max(1, keepdim=True)[1]
        seg_label_post = seg_label_post.detach()
        pesudo_bc = torch.logical_not(seg_label_post == gt_semantic_seg_pre)
        pesudo_bc = pesudo_bc.to(torch.long)
        losses = self.losses(seg_logit=bc_logit, seg_label=pesudo_bc)
        return losses


@HEADS.register_module()
class SimpleHeadBCUSL1(SimpleHeadBC):
    '''
    unsupervised loss + l1 loss
    '''
    def __init__(self, l1_weight=1., **kwargs):
    ...
    def forward_train(
        ...
        pesudo_bc = torch.logical_not(seg_label_post == gt_semantic_seg_pre)
        pesudo_bc = pesudo_bc.to(torch.long)
        bc_softmax = torch.softmax(bc_logit, dim=1)
        l1_losses = torch.mean(bc_softmax[:, 1])
        losses = self.losses(seg_logit=bc_logit, seg_label=pesudo_bc)
        losses['l1_loss_seg'] = l1_losses * self.l1_weight
        return losses


@HEADS.register_module()
class SimpleHeadBCUSL1Seg(SimpleHeadBCUSL1):
    '''
    infer bc output using seg output
    '''

    def forward_train(self, inputs, img_metas, train_cfg, gt_semantic_seg_pre, gt_semantic_seg_post=None, gt_semantic_seg=None):
        losses = dict(
    ...
    def forward_test(
        ...
        pred_sem1 = resize(gt_semantic_seg_pre.unsqueeze(1).float(), size=seg_logits_post.shape[-2:], mode='nearest')

        pred_sem2 = seg_logits_post.argmax(dim=1, keepdim=True)
        pred_bc = 1 - (pred_sem1 == pred_sem2).int()
        output_bc = torch.cat([1. - pred_bc, pred_bc], dim=1).to(seg_logits_post.dtype)
...
class SimpleConcatModule(nn.Module):
    ...
    [f2, m1] -> single layer convolutional layer with kernel_size = 1
    '''
    def __init__(self, in_channels, out_channels, kernel_size=1):
        super(SimpleConcatModule, self).__init__()
...
class DWKConcatModule(nn.Module):
    '''
    Module for extracting K representations of joint features in parallel.
    '''
    def __init__(self, in_channels, out_channels, k, norm_cfg):
        super(DWKConcatModule, self).__init__()
        self.conv1 = ConvModule(in_channels, out_channels * k, kernel_size=1, norm_cfg=norm_cfg)
        self.conv2 = ConvModule(out_channels * k, out_channels * k, kernel_size=1, norm_cfg=norm_cfg,
                                groups=k)

        self.conv1 = ConvModule(in_channels, out_channels * k, kernel_size=1, norm_cfg=norm_cfg)
        self.dw_conv = nn.Conv2d(out_channels * k, out_channels * k, kernel_size=3, padding=1, groups=out_channels * k, bias=False)
        self.conv2 = ConvModule(out_channels * k, out_channels * k, kernel_size=1, norm_cfg=norm_cfg,
                                groups=k)
...
class ContrastiveModule(nn.Module):
    ...
    def forward(self, bc, g1, f2, f1=None, m1=None):
        ...
        # cos2 = cos_similarity(g1_proj, f2_proj, dim=1).flatten()
        ...
        if f1 is not None:
            ...
            loss1_pos = torch.tensor(0.).to(loss2_pos.device)
...
class AlignedContrastiveModule(ContrastiveModule):
    '''
    Contrastive loss module that aligns the feature maps with the bc map.
    '''
    def forward(self, bc, g1, f2, f1=None, m1=None):
        B = g1.shape[0]
        H_g, W_g = g1.shape[-2:]
        ...
        f2 = resize(input=f2, size=(H, W), mode='bilinear', align_corners=self.align_corners)
        f2_proj = self.img_proj(f2)
        cos2 = nn.functional.cosine_similarity(g1_proj, f2_proj, dim=1)
        cos2_expand = cos2[:, None, ...]
        bc_H, bc_W = bc.shape[-2:]
        cos2_up = resize(cos2_expand, size=(bc_H, bc_W), mode='bilinear', align_corners=self.align_corners)
        cos2 = cos2_up.flatten()
        mask = (bc == 1).flatten()
        # bc = resize(input=bc.float(), size=(H, W), mode='nearest')
        # mask = (bc == 1).flatten()
        # cos2 = nn.functional.cosine_similarity(g1_proj, f2_proj, dim=1).flatten()
        ...
        if f1 is not None:
            f1 = resize(input=f1, size=(H, W), mode='bilinear', align_corners=self.align_corners)
            f1_proj = self.img_proj(f1)
            cos1 = nn.functional.cosine_similarity(g1_proj, f1_proj, dim=1)
            cos1_expand = cos1[:, None, ...]
            cos1_up = resize(cos1_expand, size=(bc_H, bc_W), mode='bilinear', align_corners=self.align_corners)
            cos1 = cos1_up.flatten()
            # cos1 = nn.functional.cosine_similarity(g1_proj, f1_proj, dim=1).flatten()
...
class ScaleContrastiveModule(ContrastiveModule):
    '''
    Contrastive loss module with a learnable scale factor.
    '''

    def __init__(self, **kwargs):
        logit_scale = kwargs.pop('logit_scale', None)
        super().__init__(**kwargs)
        self.logit_scale = logit_scale if logit_scale is None else nn.Parameter(torch.tensor(logit_scale))
    ...
    def forward(self, bc, g1, f2, f1=None, m1=None):
        ...
        g1 = resize(input=g1, size=(H, W), mode='nearest')
        g1_proj = self.map_proj(g1)
        ...
        mask = (bc == 1).flatten()
        # cos2 = nn.functional.cosine_similarity(g1_proj, f2_proj, dim=1).flatten()
        cos2 = cos_similarity(g1_proj, f2_proj, dim=1, scale=self.logit_scale).flatten()
...
class DistilledModule(nn.Module):
    '''
    Distillated loss module.
    '''
    def __init__(self, in_channels_img, proj_channels, align_corners):
        super().__init__()
        if in_channels_img is not None and in_channels_img != proj_channels:
            self.img_proj = nn.Conv2d(in_channels_img, proj_channels, kernel_size=1)
        else:
            self.img_proj = nn.Identity()
        self.align_corners = align_corners
    ...
    def forward(self, x, y):
        ...
        if y.shape[-2:] != x.shape[-2:]:
            y = resize(input=y, size=x.shape[-2:], mode='bilinear', align_corners=self.align_corners)

        cos_xy = nn.functional.cosine_similarity(x_proj, y, dim=1)
        l = torch.mean(1 - cos_xy)
        return {'distilled_loss': l}


class MulDistilledModule(nn.Module):
    '''
    MulDistillated loss module.
    '''
    def __init__(self, in_channels_img, proj_channels, align_corners):
        super().__init__()
        # if in_channels_img is not None and in_channels_img != proj_channels:
        ...
        # else:
        #     self.img_proj = nn.Identity()
        assert isinstance(in_channels_img, (tuple, list)), f'in_channels_img must be tuple or list, but got {type(in_channels_img)}'
        self.img_proj = nn.ModuleList()
        for i_channels in in_channels_img:
            if i_channels != proj_channels:
                self.img_proj.append(nn.Conv2d(i_channels, proj_channels, kernel_size=1))
            else:
                self.img_proj.append(nn.Identity())
        self.align_corners = align_corners
    ...
    def forward(self, x, y):
        ...
        loss = dict()
        for i, (i_x, i_img_proj) in enumerate(zip(x, self.img_proj)):
            i_x_proj = i_img_proj(i_x)

            if y.shape[-2:] != i_x.shape[-2:]:
                i_y = resize(input=y, size=i_x.shape[-2:], mode='bilinear', align_corners=self.align_corners)
            else:
                i_y = y

            cos_xy = nn.functional.cosine_similarity(i_x_proj, i_y, dim=1)
            i_l = torch.mean(1 - cos_xy)
            loss[f'{i}_distilled_loss'] = i_l

        if len(loss) == 1:
            loss['distilled_loss'] = loss.pop('0_distilled_loss')
        return loss
...
class DynamicMLP_C(nn.Module):
    ...
    def forward(self, features, weight_inputs):
        ...
        img_fea = img_fea.permute(0,3,1,2).contiguous() # (B,C,H,W)
        img_fea = self.conv3(img_fea)
        img_fea = self.ln(img_fea.permute(0,2,3,1)).permute(0,3,1,2).contiguous()
...
@HEADS.register_module()
class FHD_Map_Head(FHD_Head):
    '''
    BC head for FHD.
    Taken from https://github.com/ZSVOS/FHD/blob/f1f55a1912b0488ae8dd701bdba8584f63ef169c/mmseg/models/cd/fhd.py#L42.
    '''
    def __init__(self, map_encoder, **kwargs):
        super().__init__(**kwargs)
        self.map_encoder = build_from_cfg(map_encoder, MAP_ENCODERS)

    def forward(self, inputs, gt_semantic_seg_pre=None):
        map_feat = self.map_encoder(gt_semantic_seg_pre)
        assert len(inputs) == len(map_feat), 'len(x) != len(map_feat)'
        for i in range(len(inputs)):
            x_shape = inputs[i].shape
            map_shape = map_feat[i].shape
            # keep the same spatial size
            if x_shape[2:] != map_shape[2:]:
                map_feat[i] = resize(map_feat[i], size=x_shape[2:], mode='bilinear', align_corners=False)
            # keep the same channel number
            if x_shape[1] != map_shape[1]:
                channel_diff = x_shape[1] - map_shape[1]
                new_x_shape = list(copy.deepcopy(x_shape))
                new_x_shape[1] = channel_diff
                channel_padding = torch.zeros(tuple(new_x_shape), dtype=map_feat[i].dtype, device=map_feat[i].device)
                map_feat[i] = torch.cat([map_feat[i], channel_padding], dim=1)

        for i in range(len(inputs)):
            inputs[i] = torch.cat([inputs[i], map_feat[i]], dim=0)
...
class DWConcatModule(nn.Module):
    '''
    Module for concatenation multiple features along channel dim.
    '''
    def __init__(self, in_channels, out_channels, norm_cfg):
        super(DWConcatModule, self).__init__()
        # v1
        self.conv1 = ConvModule(in_channels, out_channels, kernel_size=1, norm_cfg=norm_cfg)
        # self.dw_conv = nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1, groups=256)
        self.dw_conv = nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1, groups=out_channels)
        self.conv2 = ConvModule(out_channels, out_channels, kernel_size=1, norm_cfg=norm_cfg)

        # v2
        # mid_channels = 256
        # self.conv1 = ConvModule(in_channels, mid_channels, kernel_size=1, norm_cfg=norm_cfg)
        # self.dw_conv = nn.Conv2d(mid_channels, mid_channels, kernel_size=3, padding=1, groups=mid_channels, bias=False)
        # self.conv2 = ConvModule(mid_channels, out_channels, kernel_size=1, norm_cfg=norm_cfg)
...
@HEADS.register_module()
class RouteHeadV1(nn.Conv2d):
    '''
    (map feat | img feat) -> 1 convolutional layer with kernel size = 1 -> weights
    '''
    ...
    def forward(self, features):
        concat_features = features if isinstance(features, torch.Tensor) else torch.cat(features, dim=1)
        return super().forward(concat_features)


@HEADS.register_module()
class RouteHeadV2(ConcatModule):
    '''
    (map feat | img feat) -> ConcatModule -> weights
    '''
    def __init__(self, in_channels, out_channels, norm_cfg, **kwargs):
        super().__init__(in_channels=in_channels, out_channels=out_channels, norm_cfg=norm_cfg)


@HEADS.register_module()
class RouteHeadV3(DWConcatModule):
    '''
    (map feat | img feat) -> DWConcatModule -> weights
    '''
    def __init__(self, in_channels, out_channels, norm_cfg, **kwargs):
        super().__init__(in_channels=in_channels, out_channels=out_channels, norm_cfg=norm_cfg)
...
@HEADS.register_module()
class MapFormerHeadAttn(MapFormerHead):
    '''
    BC for MapFormer.
    '''
    def __init__(self, route_head, fusion_channels, **kwargs
    ):
        super(MapFormerHeadAttn, self).__init__(**kwargs)

        attention_weights = []
        for s in range(len(self.in_channels)):
            attn_cfg = {'in_channels': self.in_channels[s] * 2,
    ...
    def forward_train(
        ...
        for s, module in enumerate(self.temporal_fusion_modules):
            ...
            attn_weights = self.attention_weights[s]([f1, f2]) # (B,KC, H, W)
            attn_weights = attn_weights.reshape(
                h_k.shape[0],
                self.k,
                h_k.shape[2],
