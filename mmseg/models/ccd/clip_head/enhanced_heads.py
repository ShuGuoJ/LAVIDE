...
class ContextDecoder(nn.Module):
    ...
    def forward(self, visual, text):
        ...
        if visual.ndim == 4:
            ...
            N = H * W
            flag = True
            visual = visual.reshape(B, C, N).permute(0, 2, 1).contiguous()
        else:
            B, N, C = visual.shape
...
@HEADS.register_module()
class MultiContextDecoder(nn.Module):
    def __init__(self,
                 transformer_width=256,
                 transformer_heads=1,
                 transformer_layers=1,
        ...
        super().__init__()
        context_decoders = []
        if not isinstance(visual_dim, list | tuple):
            visual_dim = (visual_dim,)
        for i in range(len(visual_dim)):
            decoder = ContextDecoder(transformer_width=transformer_width,
                                     transformer_heads=transformer_heads,
                                     transformer_layers=transformer_layers,
                                     visual_dim=visual_dim[i],
                                     text_dim=prompt_dim,
                                     dropout=dropout,
                                     **kwargs)
            context_decoders.append(decoder)
        self.context_decoders = nn.ModuleList(context_decoders)

    def forward(self, visual, text):
        if not isinstance(visual, list|tuple):
            visual = [visual]
        outs = []
        text = text.to(visual[0].device)
        for i in range(len(visual)):
            out = self.context_decoders[i](visual=visual[i], text=text)
            outs.append(out)

        return outs

# if __name__ == '__main__':
#     net = ContextDecoder(transformer_width=6,
#                          transformer_heads=1,
#                          transformer_layers=1,
#                          visual_dim=4,
#                          text_dim=2)
#     text = torch.rand(1, 3, 2)
#     visual = torch.rand(1, 4, 2, 2)
#     out = net(visual=visual, text=text)
#     print('hello world')