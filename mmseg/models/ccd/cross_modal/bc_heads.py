...
@HEADS.register_module()
class ContrastCrossModalMapFormerHead(CrossModalMapFormerHead):
    '''
    BC subhead for cross-modal MapFormer.
    '''
    ...
    def __init__(
        ...
        super(ContrastCrossModalMapFormerHead, self).__init__(
            feature_strides=feature_strides,
            map_encoder=map_encoder,
            extra_branch=extra_branch,
            k=k,
    ...
    def forward(self, inputs, gt_semantic_seg_pre):
        ...
        f2_list = x
        f2_merged = self.contrastive_img_forward(f2_list)
        cos_sim = F.cosine_similarity(f2_merged, map_features[0], dim=1)
        inv_cos_sim = -cos_sim
        out = torch.stack([cos_sim, inv_cos_sim], dim=1)
        return out
