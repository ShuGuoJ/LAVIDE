...
class BaseHeadCCD(nn.Module, metaclass=ABCMeta):
    ...
    def forward_test(
        ...
        # (2) return feat for visualization
        # output_bc, output_feat = self.bc_head.forward_test(
        #     inputs=inputs,
        #     img_metas=img_metas,
        #     test_cfg=test_cfg,
        #     gt_semantic_seg_pre=gt_semantic_seg_pre
        # )
        # return {'bc': output_bc, 'sem': output_sem, 'feat': output_feat}
...
class SemSegHeadCCD(nn.Module, metaclass=ABCMeta):
    ...
    '''
    def __init__(self, sem_head, inference_with_gt_sem_pre=False):
        super(SemSegHeadCCD, self).__init__()
        self.sem_head = builder.build_head(sem_head)
        self.inference_with_gt_sem_pre = inference_with_gt_sem_pre
    ...
    def forward_test(
        ...
        # # pred_sem1: [B, 1, H, W], pred_sem2: [B, 1, H, W]
        # pred_sem1_np, pred_sem2_np, pred_bc = pred_sem1.cpu().numpy(), pred_sem2.cpu().numpy(), pred_bc.cpu().numpy()
        # pred_sem1_np, pred_sem2_np, pred_bc = pred_sem1_np[0, 0], pred_sem2_np[0, 0], pred_bc[0, 0]
        # fig, ax = plt.subplots(1, 3)
        # ax[0].imshow(pred_sem1_np)
        # ax[1].imshow(pred_sem2_np)
        # ax[2].imshow(pred_bc)
        # plt.show()
        # print('show')
...
@HEADS.register_module()
class SemSegHeadCCD_(BaseHeadCCD):
    '''
    Head for change detection via semantic segmentation for semantic change detection head.
    '''
    ...
    def forward_test(
        ...
        pred_sem1 = resize(gt_semantic_seg_pre.unsqueeze(1).float(), size=output_sem.shape[-2:], mode='nearest')
        pred_sem2 = output_sem.argmax(dim=1, keepdim=True)
        pred_bc = 1 - (pred_sem1 == pred_sem2).int()
        output_bc = torch.cat([1. - pred_bc, pred_bc], dim=1).to(output_sem.dtype)

        return {'bc': output_bc, 'sem': output_sem}


@HEADS.register_module()
class ClipHeadCCD(BaseHeadCCD):
    '''
    Head for change detection and semantic segmentation via clip text.
    '''
    def forward_test(
        self,
        inputs,
        ...
        output_bc, output_sem = self.bc_head.forward_test(
            inputs=inputs,
            img_metas=img_metas,
            test_cfg=test_cfg,
            gt_semantic_seg_pre=gt_semantic_seg_pre
...
@HEADS.register_module()
class JointWeaklyHeadCCD(BaseHeadCCD):
    '''
    Base class with joint subheads for binary change and weakly semantic segmentation.
    '''
...
@HEADS.register_module()
class TripleHeadCCD(BaseHeadCCD):
    '''
    Base class with separate subheads for binary change, semantic segmentation, and boundary prediction
    '''

    def __init__(self, boundary_head, **kwargs):
        super(TripleHeadCCD, self).__init__(**kwargs)
        self.boundary_head = builder.build_head(boundary_head)

    def init_weights(self):
        super().init_weights()
        self.boundary_head.init_weights()
    ...
    def forward_train(
        ...
        boundary_inputs = inputs[-3:-2]
        inputs = inputs[:-3] + inputs[-2:]
        losses_sem = self.sem_head.forward_train(
            inputs,
            img_metas,
            train_cfg,
        ...
        losses_boundary = self.boundary_head.forward_train(
            boundary_inputs,
            img_metas,
            train_cfg,
            gt_semantic_seg=gt_semantic_seg,
        ...
        losses.update(add_prefix(losses_sem, 'sem'))
        losses.update(add_prefix(losses_bc, 'bc'))
        losses.update(add_prefix(losses_boundary, 'boundary'))
    ...
    def forward_test(
        ...
        output_boundary = self.boundary_head.forward_test(
            inputs = boundary_inputs,
            img_metas=img_metas,
            test_cfg=test_cfg,
            gt_semantic_seg_pre=gt_semantic_seg_pre
        )
        return {'bc': output_bc, 'sem': output_sem, 'boundary': output_boundary}


@HEADS.register_module()
class TripleHeadCCDDegradation(BaseHeadCCD):
    '''
    Base class with separate subheads for binary change, semantic segmentation, and boundary prediction
    '''

    def __init__(self, degradation_head, **kwargs):
        super(TripleHeadCCDDegradation, self).__init__(**kwargs)
        self.degradation_head = builder.build_head(degradation_head)

    def init_weights(self):
        super().init_weights()
        self.degradation_head.init_weights()
    ...
    def forward_train(
        ...
        pred_bc, bc_feat = self.bc_head.forward_test(
            inputs=inputs,
            img_metas=img_metas,
            test_cfg=train_cfg,
            gt_semantic_seg_pre=gt_semantic_seg_pre
        )
        losses_degradation = self.degradation_head.forward_train(
            [bc_feat, pred_bc],
            img_metas,
            train_cfg,
            gt_semantic_seg=gt_semantic_seg,
        ...
        losses = {}
        losses.update(add_prefix(losses_degradation, 'degradation'))
    ...
    def forward_test(
        ...
        output_bc, bc_feat = self.bc_head.forward_test(
            inputs=inputs,
            img_metas=img_metas,
            test_cfg=test_cfg,
            gt_semantic_seg_pre=gt_semantic_seg_pre
        )
        output_bc, output_sem, inst_map, inst_pred = self.degradation_head.forward_test(
            inputs = [bc_feat, output_bc, output_sem],
            img_metas=img_metas,
            test_cfg=test_cfg,
            gt_semantic_seg_pre=gt_semantic_seg_pre
        )
        return {'bc': output_bc, 'sem': output_sem, 'inst': inst_map, 'inst_pred': inst_pred}
